...
{
    public class 預留資料篩選 : 通用可篩選介面<預留資料>
    {
        ...
        public override IEnumerable<預留資料> 篩選(IEnumerable<預留資料> 資料列舉_)
        {
            IEnumerable<預留資料> 目前列舉_ = 資料列舉_;
            ...
            if (目前列舉_ != 資料列舉_)
                return 目前列舉_.DefaultIfEmpty(預留資料.空白);
            return 目前列舉_;
        }
    }
}
