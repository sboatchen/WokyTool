...
{
    public partial class 修改商品視窗 : Form
    {
        ...
        {
            this.商品列表.DataSource = Data_.ToList();
        }
        ...
        {
            bool IsAllowControl_ = (_資料 != null && _資料.編號 > 常數.空白資料編碼);

            this.品號.Enabled = IsAllowControl_;
            this.名稱.Enabled = IsAllowControl_;

            this.數量1.Enabled = IsAllowControl_;
            this.數量2.Enabled = IsAllowControl_;
            this.數量3.Enabled = IsAllowControl_;
            this.數量4.Enabled = IsAllowControl_;
            this.數量5.Enabled = IsAllowControl_;

            this.廠商.Enabled = IsAllowControl_;
            this.公司.Enabled = IsAllowControl_;

            this.大類.Enabled = IsAllowControl_;
            this.小類.Enabled = IsAllowControl_;

            this.需求1.Enabled = IsAllowControl_;
            this.需求2.Enabled = IsAllowControl_;
            this.需求3.Enabled = IsAllowControl_;
            this.需求4.Enabled = IsAllowControl_;
            this.需求5.Enabled = IsAllowControl_;
        }
        ...
        private void 商品列表_DropDown(object sender, EventArgs e)
        {
            if (商品列表.Text.Length == 0 || 商品列表.Text.Equals(商品列表.SelectedText))
            {
                商品列表.DataSource = _商品資料Listener.Data.ToList();

                // 重設資源須重設選定目標，不然會用預設值
                if (_資料 != null)
                    商品列表.SelectedValue = _資料.編號;
            }
            else
            {
                商品列表.DataSource = _商品資料Listener.Query.Where(Value => Value.名稱.Contains(商品列表.Text)).ToList();
            }
        }

        private void 商品列表_SelectionChangeCommitted(object sender, EventArgs e)
        {
            _資料 = 商品管理器.Instance.Get((int)this.商品列表.SelectedValue);
