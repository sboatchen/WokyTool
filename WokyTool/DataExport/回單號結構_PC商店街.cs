...
{
    class 回單號結構_PC商店街 : 可格式化_Csv
    {
        private static string 全速配編號 = "8";
        private static string 宅配通編號 = "4";

        protected 出貨匯入結構_PC商店街 _Data;

        public 回單號結構_PC商店街(出貨匯入結構_PC商店街 Data_)
        {
            _Data = Data_;
        }
            ...
            get
            {
                switch (_Data.配送公司)
                {
                    ...
                    case 列舉.配送公司.宅配通:
                        return 宅配通編號;
                    default:
                        MessageBox.Show("回單號結構_PC商店街 can't find 配送公司 " + _Data.配送公司.ToString(), 字串.錯誤, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return 字串.空;
                }
            }
        }
