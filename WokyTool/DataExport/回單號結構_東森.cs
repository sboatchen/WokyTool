...
namespace WokyTool.DataExport
{
    class 回單號結構_東森
    {
        ...
        public 回單號結構_東森(WokyTool.DataImport.出貨匯入結構_東森 From_)
        {
            無用 = From_.無用;
            訂單編號 = From_.訂單編號;
            ...
            { 
                ...
                case 列舉.配送公司類型.宅配通:
                    ...
                    break;
                default:
                    MessageBox.Show("回單號結構_東森 can't find 配送公司 " + From_.配送公司.ToString(), 字串.錯誤, MessageBoxButtons.OK, MessageBoxIcon.Error);
                    break;
            }
