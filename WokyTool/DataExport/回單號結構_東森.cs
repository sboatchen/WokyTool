...
{
    class 回單號結構_東森 : 可格式化_Csv
    {
        protected 出貨匯入結構_東森 _Data;

        public 回單號結構_東森(出貨匯入結構_東森 Data_)
        {
            _Data = Data_;
        }
        ...
        public string 顏色 { get { return _Data.顏色; } }
            ...
            get
            {
                switch (_Data.配送公司)
                {
                    ...
                    case 列舉.配送公司類型.宅配通:
                        return 字串.宅配通;
                    default:
                        MessageBox.Show("回單號結構_東森 can't find 配送公司 " + _Data.配送公司.ToString(), 字串.錯誤, MessageBoxButtons.OK, MessageBoxIcon.Error);
                        return 字串.空;
                }
            }
        }
