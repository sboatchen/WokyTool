...
{
    public class 平台訂單自定義_百利市 : 平台訂單自定義介面
    {
        ...
        {
            var Items_ = 資料_.Select(Value => new 平台訂單回單轉換_百利市(Value));

            String Title_ = String.Format("百利市回單_{0}", 時間.目前日期);
            函式.ExportExcel<平台訂單回單轉換_百利市>(Title_, Items_);
        }
    }
}
