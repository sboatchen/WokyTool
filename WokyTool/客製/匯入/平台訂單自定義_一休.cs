...
namespace WokyTool.客製
{
    public class 平台訂單自定義_一休 : 平台訂單自定義介面
    {
        ...
        {
            var Items_ = 資料_.Select(Value => new 平台訂單回單轉換_一休(Value));

            String Title_ = String.Format("一休回單_{0}", 時間.目前日期);
            函式.ExportExcel<平台訂單回單轉換_一休>(Title_, Items_);
            ...
            Title_ = String.Format("一休總金額_{0}", 時間.目前日期);
