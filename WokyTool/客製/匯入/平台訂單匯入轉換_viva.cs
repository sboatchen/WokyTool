...
{
    public class 平台訂單匯入轉換_viva : 平台訂單自定義介面, 可讀出介面_EXCEL<平台訂單匯入資料>
    {
        ...
        public 平台訂單匯入轉換_viva(公司資料 公司_)
        {
            公司 = 公司_;
            客戶 = 客戶資料管理器.獨體.Get("viva");
        }
        ...
        public IEnumerable<平台訂單匯入資料> 讀出資料(string[] 資料列_)
        {
            ...
            string 手機_ = 資料列_[10].轉成字串();
            string 電話_ = 資料列_[9].轉成字串();
            string 地址_ = 資料列_[8].轉成字串();

            string 商品識別_ = 資料列_[2].轉成字串();
