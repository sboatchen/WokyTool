...
{
    public class 平台訂單自定義_Friday : 平台訂單自定義介面
    {
        ...
        public override IEnumerable<平台訂單匯入資料> 轉換(動態匯入檔案結構 檔案_)
        {
            ...
            {
                String 出貨狀態_ = 動態資料_.Get<String>(平台訂單列舉.匯入需求欄位.檢查_出貨狀態.ToString());
                if (出貨處理中.Equals(出貨狀態_) == false)
                    continue;

                列舉.訂單處理狀態 處理狀態_ = 列舉.訂單處理狀態.新增;
                DateTime 出貨日期_ = 動態資料_.Get<DateTime>(平台訂單列舉.匯入需求欄位.檢查_出貨日期.ToString());
                if (出貨日期_.CompareTo(時間.五天後) > 0)
                {
                    處理狀態_ = 列舉.訂單處理狀態.忽略;
                }
