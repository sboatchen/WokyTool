...
{
    public class 平台訂單自定義_myfone //: 平台訂單自定義介面
    {
        ...
        {
            var Items_ = 資料列舉_.Select(Value => new 平台訂單回單轉換_myfone(Value));

            String Title_ = String.Format("myfone回單_{0}", 時間.目前日期);
            舊函式.ExportCSV<平台訂單回單轉換_myfone>(Title_, Items_);
        }*/
    }
}
