...
{
    public class 平台訂單自定義_Momo //: 平台訂單自定義介面
    {
        /*private static string 交換配送訂單 = "交換配送訂單";
        private static string 來回件 = " (來回件)";
        private static int 姓名特別前綴索引 = -1;

        public override IEnumerable<平台訂單新增匯入資料> 轉換(動態匯入檔案結構 檔案_)
        {
            ...
            foreach (var 動態資料_ in 檔案_.內容)
            {
                ...
                // 訂單類別為 "交換配送訂單" 須於姓名加上 (來回件) 並指定宅配公司為宅配通
                String 訂單類別_ = 動態資料_.Get<String>(平台訂單列舉.匯入需求欄位.檢查_訂單類別.ToString());
                列舉.配送公司 配送公司_ = 列舉.配送公司.無;
                Dictionary<int, object> 額外資訊_ = 動態資料_.詳細;
                if (交換配送訂單.CompareTo(訂單類別_) == 0)
                {
                    額外資訊_.Add(姓名特別前綴索引, 來回件);
                    配送公司_ = 列舉.配送公司.宅配通;
                }
                ...
                var 平台訂單新增匯入資料_ = new 平台訂單新增匯入資料
                {
                    處理狀態 = 處理狀態_,
                    處理時間 = 處理時間_,
                    公司 = 公司_,
                ...
                平台訂單新增匯入資料_.商品 = 商品資料管理器.獨體.取得(客戶_.編號, 商品識別_);
                平台訂單新增匯入資料_.處理器 = this;

                yield return 平台訂單新增匯入資料_;
            }
        }

        public override String 取得配送姓名(平台訂單新增資料 資料_)
        {
            Object 備註_ = null;
            if(資料_.額外資訊.TryGetValue(姓名特別前綴索引, out 備註_) == false)
                return 資料_.姓名;
