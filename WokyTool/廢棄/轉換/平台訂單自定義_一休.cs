...
{
    public class 平台訂單自定義_一休 //: 平台訂單自定義介面
    {
        ...
        public override IEnumerable<平台訂單新增匯入資料> 轉換(動態匯入檔案結構 檔案_)
        {
            ...
            foreach (var 動態資料_ in 檔案_.內容)
            {
                ...
                else 
                {
                    ...
                    商品資料_ = 商品資料管理器.獨體.取得(客戶_.編號, 商品識別_);
                }
        ...
        {
            {
                var 轉換_ = new 平台訂單回單轉換_一休(資料列舉_);

                String 標題_ = String.Format("一休回單_{0}", 時間.目前日期);
                檔案.詢問並寫入(標題_, 轉換_);
            }

            {
                ...
                String 標題_ = String.Format("一休總金額_{0}", 時間.目前日期);
                檔案.詢問並寫入(標題_, 轉換_);
            }
