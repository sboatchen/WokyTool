...
{
    public class 平台訂單資料篩選 : 通用可處理資料篩選介面<平台訂單資料>
    {
        ...
        public override IEnumerable<平台訂單資料> 篩選(IEnumerable<平台訂單資料> 資料列舉_)
        {
            IEnumerable<平台訂單資料> 目前列舉_ = base.篩選(資料列舉_);
            ...
            if (單品資料.不篩選 != _單品)
                目前列舉_ = 目前列舉_.Where(Value => Value.商品.組成.Where(Value2 => Value2.單品 == _單品).Any());
            ...
            if (null != _發票號碼)
                目前列舉_ = 目前列舉_.Where(Value => Value.發票號碼 != null && Value.發票號碼.Contains(_發票號碼));
            if (null != _備註)
            ...
            if (目前列舉_ != 資料列舉_)
                return 目前列舉_.DefaultIfEmpty(平台訂單資料.空白);
            return 目前列舉_;
        }
    }
}
