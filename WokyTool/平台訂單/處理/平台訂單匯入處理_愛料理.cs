...
{
    public class 平台訂單匯入處理_愛料理 : 平台訂單匯入處理介面, 可讀出介面_CSV<平台訂單新增匯入資料>
    {
        ...
        public IEnumerable<平台訂單新增匯入資料> 讀出資料(string[] 資料列_)
        {
            ...
            string 手機_ = 資料列_[7].轉成字串();
            string 地址_ = 資料列_[8].轉成字串();

            string 商品名稱_ = 資料列_[10].轉成字串();
            string 款式_ = 資料列_[11].轉成字串();
            string 商品識別_ = 函式.取得商品識別(商品名稱_, 款式_);
            商品資料 商品_ = 商品資料管理器.獨體.取得(客戶.編號, 商品識別_);

            int 數量_ = 資料列_[12].轉成整數();

            string 備註_ = 資料列_[21].轉成字串();

            string 發票號碼_ = 資料列_[0].轉成字串();
        ...
        {
            var 轉換_ = new 平台訂單回單轉換_愛料理(資料列舉_);
