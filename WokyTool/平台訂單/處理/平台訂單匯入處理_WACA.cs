...
{
    public class 平台訂單匯入處理_WACA : 平台訂單匯入處理介面, 可讀出介面_EXCEL<平台訂單新增匯入資料>
    {
        ...
        public 平台訂單匯入處理_WACA()
        {
            客戶 = 客戶資料管理器.獨體.取得("WACA");
        }
        ...
        public IEnumerable<平台訂單新增匯入資料> 讀出資料(string[] 資料列_)
        {
            ...
            string 姓名_ = 資料列_[1].轉成字串();
            string 電話_ = 資料列_[2].轉成字串();
            string 地址_ = 資料列_[3].轉成字串();

            string 商品識別_ = 資料列_[9].轉成字串();
            商品資料 商品_ = 商品資料管理器.獨體.取得(客戶.編號, 商品識別_);

            int 數量_ = 資料列_[10].轉成整數();

            string 備註_ = 資料列_[4].轉成字串();
            
            string 店家備註_ = 資料列_[13].轉成字串();
            if(店家備註_ != null)
            {
                if (備註_ != null)
                    備註_ += ", " + 店家備註_;
        ...
        {
            var 轉換_ = new 平台訂單回單轉換_WACA(資料列舉_);
