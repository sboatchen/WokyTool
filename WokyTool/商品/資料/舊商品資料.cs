...
namespace WokyTool.商品
{
    ...
    public class 舊商品資料 : MyKeepableData<舊商品資料>
    {
        [JsonProperty]
        ...
        private static 舊商品資料 _折扣 = new 舊商品資料
        {
            編號 = 常數.商品折扣資料編碼,
        ...
        public override 舊商品資料 拷貝()
        {
            舊商品資料 Data_ = new 舊商品資料
            {
                編號 = this.編號,
        ...
        public override void 覆蓋(舊商品資料 Data_)
        {
            編號 = Data_.編號;
        ...
        {
            if (大類.編號是否合法() == false)
                throw new Exception("舊商品資料:大類編號不合法:" + 大類編號);

            if (小類.編號是否合法() == false)
                throw new Exception("舊商品資料:小類編號不合法:" + 小類編號);

            if (公司.編號是否合法() == false)
                throw new Exception("舊商品資料:公司編號不合法:" + 公司編號);

            if (客戶.編號是否合法() == false)
                throw new Exception("舊商品資料:客戶編號不合法:" + 客戶編號);

            if (需求1.編號是否合法() == false)
                throw new Exception("舊商品資料:需求編號1不合法:" + 需求編號1);

            if (需求2.編號是否合法() == false)
                throw new Exception("舊商品資料:需求編號2不合法:" + 需求編號2);

            if (需求3.編號是否合法() == false)
                throw new Exception("舊商品資料:需求編號3不合法:" + 需求編號3);

            if (需求4.編號是否合法() == false)
                throw new Exception("舊商品資料:需求編號4不合法:" + 需求編號4);

            if (需求5.編號是否合法() == false)
                throw new Exception("舊商品資料:需求編號5不合法:" + 需求編號5);

            //if (String.IsNullOrEmpty(品號))
            //    throw new Exception("舊商品資料:品號不合法:" + this.ToString());

            if (String.IsNullOrEmpty(名稱))
                throw new Exception("舊商品資料:名稱不合法:" + this.ToString());

            if (數量1 < 0 || 數量2 < 0 || 數量3 < 0 || 數量4 < 0 || 數量5 < 0)
                throw new Exception("舊商品資料:數量不合法:" + this.ToString());
        }
        ...
        /*public bool Sell(bool 寄庫出貨_, int 數量_)
        {
            ...
            //if (數量_ <= 0)
            //{
            //    MessageBox.Show("舊商品資料::Sell 總數量參數不合法，請通知苦逼程式,", 字串.錯誤, MessageBoxButtons.OK, MessageBoxIcon.Error);
            //    return false;
            //}
