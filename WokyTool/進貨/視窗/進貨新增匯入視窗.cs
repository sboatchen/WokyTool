...
namespace WokyTool.進貨
{
    public partial class 進貨新增匯入視窗 : 匯入視窗
    {
        ...
        {
            var Items_ = _進貨新增匯入管理器.可編輯BList.Where(Value => Value.錯誤訊息 != null)
                                .Select(Value => new 進貨新增錯誤匯出轉換(Value));

            string Title_ = String.Format("進貨新增錯誤匯出_{0}", 時間.目前日期);
            函式.ExportExcel<進貨新增錯誤匯出轉換>(Title_, Items_);
        }
